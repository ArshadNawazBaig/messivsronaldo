...
export function CareerTimeline() {
  ...
  return <section className={styles.panel} aria-label={t("Career timeline")} data-testid="career-timeline">
    ...
    <div className={styles.summary} aria-label={t("Selected range totals")}><div><span>Messi</span><strong className={styles.messi} data-testid="timeline-messi-total">{fmt(totals.messi)}</strong></div><div><span>Ronaldo</span><strong className={styles.ronaldo} data-testid="timeline-ronaldo-total">{fmt(totals.ronaldo)}</strong></div><p className={styles.muted}>{t("Totals for the selected years")}</p></div>
    ...
    <div className={styles.selectedYear} aria-live="polite"><b>{active.year}</b><span className={styles.messi}>Messi <strong>{fmt(active.values.messi)}</strong></span><span className={styles.ronaldo}>Ronaldo <strong>{fmt(active.values.ronaldo)}</strong></span><Link className="text-link" href={`/seasons/${active.year}`}>{t("View original record")}</Link></div>
    <div className={styles.actions}><ToolShare key={hash(state)} hash={hash(state)}/></div>
    <details className={styles.tableWrap}><summary>{t("Open the data table")}</summary><table className={styles.table}><caption>{t(metricLabels[state.metric])} · {t(state.cumulative ? "Cumulative within range" : "Annual totals")}</caption><thead><tr><th scope="col">{t("Year")}</th><th scope="col">Messi</th><th scope="col">Ronaldo</th></tr></thead><tbody>{rows.map(row => <tr key={row.year}><th scope="row">{row.year}</th><td>{fmt(row.values.messi)}</td><td>{fmt(row.values.ronaldo)}</td></tr>)}</tbody></table></details>
