...
export async function CurrentHighlights() {
    ...
    return <section className="current-strip" aria-label={t("2026 highlights")}>{highlights.map(item => <Link className="current-tile" href={item.href} key={item.href}><span className="current-title">{t(item.title)}<ArrowUpRight size={15}/></span><strong><span className="messi-text"><small>{t("Messi")}</small>{t(item.values.messi)}</span><span className="ronaldo-text"><small>{t("Ronaldo")}</small>{t(item.values.ronaldo)}</span></strong><p>{t(item.detail)}</p></Link>)}</section>;
}
