...
export function SeasonExplorer({ selected = "all" }: {
    ...
    return <section className="season-explorer"><div className="season-controls panel"><div className="scope-tabs" role="group" aria-label={t("Season competition")}><button aria-pressed={competition === "league"} className={competition === "league" ? "selected" : ""} onClick={() => update("league", rate)}>{t("La Liga")}</button><button aria-pressed={competition === "ucl"} className={competition === "ucl" ? "selected" : ""} onClick={() => update("ucl", rate)}>{t("Champions League")}</button></div><div className="season-select"><Select id="season-picker" label={t("Season")} menuLabel={t("Shared Spanish seasons")} value={selected} icon={CalendarDays} onValueChange={value => router.push(`${value === "all" ? "/seasons" : `/seasons/${value}`}#competition=${competition}&rate=${rate ? "1" : "0"}`)} options={[{ value: "all", label: "All years & seasons" }, ...seasons.map(s => ({ value: s.slug, label: s.label }))]}/></div><label className="checkbox-label"><input type="checkbox" checked={rate} onChange={event => update(competition, event.target.checked)}/>{t("Per appearance")}</label></div><div className="calendar-matchup"><PlayerMatchup values={{
        ...
        }} label={t(rate ? "GOALS / APPEARANCE" : "TOTAL GOALS")} accessibleLabel={t(rate ? "goals per appearance" : "goals")} context={t(`${selected === "all" ? "2009/10–2017/18" : rows[0].label} · ${t(name)}`)} details={{ messi: t("{0} appearances", { "0": total.messi.appearances }), ronaldo: t("{0} appearances", { "0": total.ronaldo.appearances }) }} decimals={rate ? 2 : 0} exportData={{ title: rate ? "Goals per appearance" : "Goals", context: `${selected === "all" ? "2009/10–2017/18" : rows[0].label} · ${name}`, note: "Historical season archive · Qualifying rounds excluded" }}/></div><div className="panel season-chart"><div className="panel-heading"><div><span className="section-kicker">{t("THE SAME SEASONS. THE SAME COMPETITION.")}</span><h2>{t(selected === "all" ? "Nine seasons, side by side." : `${rows[0].label}, side by side.`)}</h2></div><div className="chart-legend"><span><i className="legend-dot messi-dot"/>{t("Messi")}</span><span><i className="legend-dot ronaldo-dot"/>{t("Ronaldo")}</span></div></div><div className="season-bar-chart">{rows.map(row => <div className="season-bar-row" key={row.slug}><Link href={`/seasons/${row.slug}#competition=${competition}&rate=${rate ? "1" : "0"}`}>{t(row.label)}</Link><div><StatImageButton stat={{ title: rate ? "Goals per appearance" : "Goals", context: `${row.label} · ${name}`, values: { messi: rate ? row[competition].messi.goals / row[competition].messi.appearances : row[competition].messi.goals, ronaldo: rate ? row[competition].ronaldo.goals / row[competition].ronaldo.appearances : row[competition].ronaldo.goals }, decimals: rate ? 2 : 0, note: "Historical season archive · Qualifying rounds excluded" }}/>{(["messi", "ronaldo"] as const).map(player => {
