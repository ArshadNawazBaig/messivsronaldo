...
export function PlayerMatchup({ values, label, accessibleLabel, context, details, decimals = 0 }: PlayerMatchupProps) {
  return <div className="player-matchup">
    {(["messi", "ronaldo"] as const).map(id => {
      ...
      const score = value === null ? "—" : value.toLocaleString("en-US", { maximumFractionDigits: decimals, minimumFractionDigits: decimals });
      return <article className={`player-card ${id}`} key={id} aria-label={`${player.name}: ${value === null ? "unavailable" : value.toFixed(decimals)} ${accessibleLabel}`}>
        <Link className="player-portrait" href={`/players/${id}`} aria-label={`View ${player.name}'s profile`}>
          ...
          <div className="player-card-copy">
            ...
            <div className="player-score"><span className={`big-score${score.length > 4 ? " is-wide-score" : ""}`}>{score}</span><div><span>{label}</span><span>{context}</span>{details && <span>{details[id]}</span>}</div></div>
