...
export function Select({ id, label, menuLabel, value, onValueChange, options, icon: Icon, className = "" }: SelectProps) {
  ...
  return <SelectPrimitive.Root value={value} onValueChange={onValueChange}>
    <SelectPrimitive.Trigger id={id} className={`select-trigger ${className}`} aria-label={label}>
      {Icon && <Icon size={16} className="select-leading-icon" aria-hidden="true" />}
      <span className="select-value"><SelectPrimitive.Value>{selected?.label}</SelectPrimitive.Value></span>
      <SelectPrimitive.Icon className="select-chevron"><ChevronDown size={15} aria-hidden="true" /></SelectPrimitive.Icon>
    </SelectPrimitive.Trigger>
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content ref={menuRef} className="select-content" position="popper" sideOffset={8} collisionPadding={12} aria-label={label}>
        ...
        <SelectPrimitive.Viewport className="select-viewport">
          <SelectPrimitive.Group>
            <SelectPrimitive.Label className="select-menu-label">{menuLabel ?? label}</SelectPrimitive.Label>
            {options.map(option => <SelectPrimitive.Item key={option.value} value={option.value} disabled={option.disabled} tabIndex={option.disabled ? undefined : option.value === value ? 0 : -1} textValue={option.label} aria-label={option.label} className="select-option">
              <SelectPrimitive.ItemText>{option.label}</SelectPrimitive.ItemText>
              {option.badge && <span className="select-option-badge" aria-hidden="true">{option.badge}</span>}
