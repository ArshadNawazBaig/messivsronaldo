...
import { ArrowRight, ArrowUpRight, Check, ChevronDown, CircleHelp, Download, Expand, Globe2, Info, Link2, ShieldCheck, SlidersHorizontal, Star, Trophy, X } from "lucide-react";
...
export function Comparison({ initialScope = "career", compact = false, initialGroup = "overview" }: { initialScope?: ScopeId; compact?: boolean; initialGroup?: MetricGroup | "all" }) {
  ...
  return <section className={`comparison ${compact ? "comparison-compact" : ""}`} id="comparison" aria-label="Interactive player comparison">
    ...
    <div className="numbers-panel panel"><div className="panel-heading"><div><span className="section-kicker">THE HEAD-TO-HEAD</span><h2>By the numbers<span className="heading-dot">.</span></h2></div><div className="panel-actions"><button className="icon-button" onClick={download} aria-label="Download comparison CSV" title="Download CSV"><Download size={17} /></button><button className="small-button" onClick={share}>{copied ? <Check size={14} /> : <Link2 size={14} />}<span aria-live="polite">{copied ? "Link copied" : "Share comparison"}</span></button></div></div>
      ...
      <div className="stats-footnote"><span><ShieldCheck size={14} />Traceable figures. Tap any metric for its source.</span><Link href="/methodology">How we count <ArrowRight size={13} /></Link></div>
    </div>
...
export function ExploreCards() {
  return <section className="explore-section"><div className="section-title-row"><div><span className="section-kicker">FOLLOW YOUR CURIOSITY</span><h2>There’s more to the story<span className="heading-dot">.</span></h2></div><Link href="/compare" className="text-link">Explore all comparisons <ArrowRight size={15} /></Link></div><div className="explore-grid"><Link href="/champions-league" className="explore-card"><span className="explore-icon blue-icon"><Trophy size={22} /></span><span className="explore-label">EUROPE’S BIGGEST STAGE</span><h3>Champions League</h3><p>The goals. The creation. The European legacy.</p><span className="explore-arrow"><ArrowUpRight size={20} /></span></Link><Link href="/la-liga" className="explore-card"><span className="explore-icon coral-icon"><Expand size={22} /></span><span className="explore-label">THE SPANISH CHAPTER</span><h3>La Liga careers</h3><p>A shared league. Two very different journeys.</p><span className="explore-arrow"><ArrowUpRight size={20} /></span></Link><Link href="/seasons" className="explore-card"><span className="explore-icon lime-icon"><SlidersHorizontal size={22} /></span><span className="explore-label">EVERY CHAPTER, THROUGH 2026</span><h3>Year by year</h3><p>Twenty-five calendar years. Club and country. One timeline.</p><span className="explore-arrow"><ArrowUpRight size={20} /></span></Link></div></section>;
}
