...
import { numberLocales, type Locale } from "@/lib/i18n/config";
...
export function I18nProvider({ locale, messages, children }: { locale: Locale; messages: Messages; children: ReactNode }) {
  const value = useMemo(() => ({ locale, numberLocale: numberLocales[locale], t: createTranslator(locale, messages) }), [locale, messages]);
