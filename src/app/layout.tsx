...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang={locale} dir={locale === "ar" ? "rtl" : "ltr"} data-theme="light" suppressHydrationWarning><head><ThemeInitializer /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd({ "@context": "https://schema.org", "@type": "WebSite", "@id": `${siteUrl}/#website`, name: siteName, alternateName: "Messi vs Ronaldo 17", publisher: { "@type": "Organization", "@id": `${siteUrl}/#publisher`, name: siteName, url: siteUrl, logo: `${siteUrl}/icon.svg` }, url: siteUrl, description: t("An independent, source-transparent Messi and Ronaldo comparison publication."), inLanguage: locale }) }} /></head><body className={`${inter.variable} ${display.variable}`}><I18nProvider locale={locale} messages={messages}><DataProvider value={await getPublishedData()}><AdminExportProvider admin={admin}><SiteShell>{children}</SiteShell></AdminExportProvider></DataProvider></I18nProvider></body></html>;
}
