...
export default defineConfig({
  ...
  webServer:{command:"npm run start -- --port 3002",url:"http://localhost:3002",reuseExistingServer:false,timeout:60000,env:{ADMIN_DATABASE_PATH:".artifacts/admin-integration.sqlite",ADMIN_PASSWORD_HASH:`${salt}:${scryptSync(password,salt,64).toString("hex")}`,ADMIN_SESSION_SECRET:"integration-test-secret-only-at-least-32-characters",NEXT_PUBLIC_SITE_URL:"http://localhost:3002",SITE_INDEXABLE:"false"}},
});
